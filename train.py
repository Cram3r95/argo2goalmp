...
parser.add_argument(
    "--use_map", default=True, type=bool)
parser.add_argument(
...
parser.add_argument(
    "--exp_name", default="test", type=str)
parser.add_argument(
...
def main():
    ...
    config, Dataset, collate_fn, net, loss, post_process, opt = model.get_model(args.exp_name,
...
def train(args, epoch, config, train_loader, net, loss, post_process, opt, val_loader=None):
    ...
    for i, data in tqdm(enumerate(train_loader),disable=hvd.rank()):
        ...
        if DEBUG: print(f"2. Loss time: {time.time()-start}")
        
        start = time.time()
        opt.zero_grad()
        loss_out["loss"].backward()
        lr = opt.step(epoch)
        if DEBUG: print(f"3. Backprop and optimizer time: {time.time()-start}")
        ...
        if epoch >= config["start_val_epoch"] and num_iters % val_iters == 0:
            val(args, config, val_loader, net, loss, post_process, epoch)

        if epoch >= config["num_epochs"]:
            val(args, config, val_loader, net, loss, post_process, epoch)
            return
        if DEBUG: print(f"4. Postprocess time: {time.time()-start}")
        end_iteration = time.time()
        if DEBUG: print(f"5. Iteration time: {end_iteration-start_iteration}")
